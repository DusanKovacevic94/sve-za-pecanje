...
import { S3Client, CreateBucketCommand, PutObjectCommand, GetObjectCommand, DeleteObjectCommand, ListObjectsV2Command } from '@aws-sdk/client-s3'
...
// Only called with credentials generated by the disposable integration harness.
export async function snapshotOwnedStorage(rootPassword: string) {
  assert.equal(process.env.CMS_ENV, 'test')
  assert.equal(new URL(process.env.CMS_S3_ENDPOINT!).hostname, '127.0.0.1')
  const client = new S3Client({ ...storageEnvironment().config, credentials: { accessKeyId: 'minioadmin', secretAccessKey: rootPassword } })
  const objects: unknown[] = []
  try {
    for (const Bucket of [CMS_MEDIA_BUCKET, 'fishing-marketplace', 'private-exports']) {
      let ContinuationToken: string | undefined
      do {
        const result = await client.send(new ListObjectsV2Command({ Bucket, ContinuationToken }))
        objects.push(...(result.Contents || []).map(({ Key, ETag, Size }) => ({ Bucket, Key, ETag, Size })))
