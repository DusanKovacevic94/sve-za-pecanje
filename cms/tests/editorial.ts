...
export async function editorialChecks(baseURL: string, adminCookie: string, mailURL: string) {
  ...
  const draft = (await edit('/posts?draft=true', 'POST', { ...fields, _status: 'draft' })).doc
  ...
  await edit(`/posts/${draft.id}?draft=true&autosave=true`, 'PATCH', { title: 'PRIVATE_REVISION', body: paragraph('PRIVATE_DRAFT_BODY'), _status: 'draft' })
  assert.equal((await edit(`/posts/${draft.id}?draft=true`)).title, 'PRIVATE_REVISION')
  for (const suffix of ['', '?draft=true', '?draft=true&depth=10&showHiddenFields=true', '?draft=true&select[title]=true&select[internalNotes]=true']) {
