...
function ReviewCard({ review, direction }: { review: ReviewItem; direction: "received" | "given" }) {
  ...
  return (
    <article className="rounded-xl border border-sand-200 bg-white p-5 shadow-soft">
      ...
      {review.comment ? <p className="mt-3 whitespace-pre-line text-sm text-ink-700">{review.comment}</p> : null}
      <p className="mt-3 text-xs font-semibold text-ink-500">{formatDate(review.created_at)}</p>
    </article>
  );
}
...
export default async function ReviewsPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-extrabold">Ocene</h1>
      <p className="mt-2 text-ink-600">Ocene posle završenih prodaja.</p>

      {reviews.data.pending.length ? (
        <section className="mt-6">
          <h2 className="text-xl font-extrabold">Čekaju vašu ocenu</h2>
      ...
      <section className="mt-6">
        <h2 className="text-xl font-extrabold">Primljene ocene</h2>
        <div className="mt-4 space-y-4">
          {reviews.data.received.length ? (
            reviews.data.received.map((review) => <ReviewCard key={review.id} review={review} direction="received" />)
          ) : (
            <div className="rounded-xl border border-sand-200 bg-white p-6 text-ink-600">Još nemate primljene ocene.</div>
          )}
        </div>
      </section>

      <section className="mt-6">
        <h2 className="text-xl font-extrabold">Date ocene</h2>
        <div className="mt-4 space-y-4">
          {reviews.data.given.length ? (
            reviews.data.given.map((review) => <ReviewCard key={review.id} review={review} direction="given" />)
          ) : (
            <div className="rounded-xl border border-sand-200 bg-white p-6 text-ink-600">Još niste ostavili ocenu.</div>
          )}
        </div>
      </section>
    </div>
