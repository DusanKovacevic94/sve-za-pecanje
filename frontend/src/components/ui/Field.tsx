  ...
  return (
    <>
      <input
        ...
        className={`focus-ring min-h-11 w-full min-w-0 rounded-xl border bg-white px-3 py-2 text-sm motion-safe:transition-colors ${
  ...
  return (
    <>
      <textarea
        ...
        aria-describedby={describedBy}
        aria-invalid={error ? true : props["aria-invalid"]}
        className={`focus-ring min-h-32 w-full min-w-0 rounded-xl border bg-white px-3 py-2 text-sm motion-safe:transition-colors ${
          error ? "border-red-400" : "border-sand-300 hover:border-river-300"
        } ${className ?? ""}`}
      />
      {error ? <FieldError id={errorId ?? `${props.id}-error`}>{error}</FieldError> : null}
