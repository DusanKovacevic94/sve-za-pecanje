...
export default defineConfig({
  ...
  webServer: [
    {
      command: `sh -c 'rm -f ./e2e.db && DATABASE_URL=sqlite:///./e2e.db APP_ENV=test LISTING_REVIEW_MODE=auto RATE_LIMIT_ENABLED=false RESEND_API_KEY= SECRET_KEY=e2e-secret-key-123456789012345678901234 JWT_SECRET=e2e-jwt-secret-123456789012345678901234 uv run python -m alembic upgrade head && DATABASE_URL=sqlite:///./e2e.db APP_ENV=test LISTING_REVIEW_MODE=auto RATE_LIMIT_ENABLED=false RESEND_API_KEY= SECRET_KEY=e2e-secret-key-123456789012345678901234 JWT_SECRET=e2e-jwt-secret-123456789012345678901234 uv run python -m uvicorn app.main:app --host 127.0.0.1 --port ${backendPort}'`,
      cwd: "../backend",
