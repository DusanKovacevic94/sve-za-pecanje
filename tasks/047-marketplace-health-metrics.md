...
- [ ] Define an allowlisted event taxonomy for searches, zero-result searches, listing
  views, favorites, saved searches, conversations started, listings published and approved,
  and listings marked sold
- [ ] Record authenticated business events inside backend services so browser analytics or
  ad blockers cannot remove critical marketplace data
- [ ] Add a rate-limited anonymous event endpoint only for events that cannot be derived
  server-side; require a client event ID and deduplicate retries
- [ ] Normalize search terms, avoid raw PII in event properties, and hash anonymous
  identifiers using the existing privacy approach
- [ ] Add daily marketplace aggregates with optional category breakdown:
...
- [ ] Add an idempotent worker task that builds or repairs aggregates for a date range
- [ ] Add an admin API and dashboard with 7-, 30-, and 90-day ranges, previous-period
  comparison, category filters, and CSV export
- [ ] Retain raw events for 90 days and daily aggregates for 24 months
- [ ] Document each metric's formula, source event, exclusions, and timezone
- [ ] Add unit, integration, authorization, aggregation, timezone, and duplicate-event tests
